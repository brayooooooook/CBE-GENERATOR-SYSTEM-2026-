...
export async function exportClassRankingPDF(data: SchoolAnalyticsData, school: School) {
  ...
  const tableData = data.class_rankings.map((c) => [
    ...
    `${Math.round(c.mean_marks)} / ${c.max_obtainable_marks}`,
  ...
  await savePdf(doc, `Class_Ranking_Report_${(data.education_level_title || 'Level').replace(/\s+/g, '_')}_${data.exam.exam_name.replace(/\s+/g, '_')}.pdf`);
}
...
export async function exportStreamRankingPDF(data: SchoolAnalyticsData, school: School) {
  ...
  const tableData = data.stream_rankings.map((st) => [
    ...
    `${Math.round(st.mean_marks)} / ${st.max_obtainable_marks}`,
  ...
  await savePdf(doc, `Stream_Ranking_Report_${(data.education_level_title || 'Level').replace(/\s+/g, '_')}_${data.exam.exam_name.replace(/\s+/g, '_')}.pdf`);
}
...
export async function exportSubjectAnalysisPDF(data: SchoolAnalyticsData, school: School) {
  ...
  await savePdf(doc, `Subject_Analysis_Report_${data.exam.exam_name.replace(/\s+/g, '_')}.pdf`);
}
...
export async function exportComprehensiveAnalyticsPDF(data: SchoolAnalyticsData, comp: ExaminationComparisonData | null, school: School) {
  ...
  await savePdf(doc, `Comprehensive_Analytics_${data.exam.exam_name.replace(/\s+/g, '_')}.pdf`);
}
