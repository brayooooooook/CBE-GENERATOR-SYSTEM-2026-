...
(async () => {
  ...
  await runTest('Integer total: 450 total marks serialises strictly as "450" in CSV', async () => {
    ...
    // Columns: SERIAL, ADM NO, NAME, ASS NO, STREAM, STR POS, OVR POS, PRV STR POS, PRV OVR POS, ENG, KIS, MATH, INT-SCI, SST, TOTAL MARKS
    const totalMarksCell = cells[14];
  ...
  await runTest('Existing CSV column header structure and order remains intact', async () => {
    ...
    const expectedHeaders = [
      'SERIAL NO', 'ADM NO', 'LEARNER NAME', 'ASS NO', 'STREAM',
