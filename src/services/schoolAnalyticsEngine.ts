...
export function calculateSchoolAnalytics(
  ...
  assessedStudents.forEach((std) => {
    ...
    const totalMarks = validStdMarks.reduce((sum, info) => sum + info.rawScore!, 0);
    const sumPercentage = validStdMarks.reduce((sum, info) => sum + info.percentage!, 0);
    const isComplete = stdReqCount > 0 ? count >= stdReqCount : true;

    const avgMarks = count > 0 ? Math.round((sumPercentage / count) * 100) / 100 : 0;
    const meanPercentage = avgMarks;

    let totalPoints = 0;
    validStdMarks.forEach((info) => {
      const g = getGradeForMark(info.percentage!, grades);
  ...
  // Separate complete students for merit ranking using KNEC tie-breaking standards
  const completeLearners = studentStatsList.filter((s) => s.is_complete !== false);
  completeLearners.sort((a, b) => {
  ...
  applyCompetitionRanking(completeLearners, isMeritTie, (item, rank) => {
    item.rank = rank;
  });
  ...
  if (totalAssessed > 0) {
    ...
    overallSchoolMean = Math.round((sumTotalMarks / totalAssessed) * 100) / 100;
    overallSchoolMeanPct = Math.round((sumPct / totalAssessed) * 100) / 100;
  ...
  classGroupMap.forEach((learners, cName) => {
    ...
    const meanM = count > 0 ? Math.round((totMarks / count) * 100) / 100 : 0;
    const meanPct = count > 0 ? Math.round((learners.reduce((sum, l) => sum + l.mean_percentage, 0) / count) * 100) / 100 : 0;
  ...
  classRankings.sort((a, b) => b.mean_points - a.mean_points || b.mean_marks - a.mean_marks);
  applyCompetitionRanking(
    classRankings,
    (a, b) => Math.round(a.mean_points * 100) === Math.round(b.mean_points * 100) && Math.round(a.mean_marks * 100) === Math.round(b.mean_marks * 100),
    (c, rank) => { c.rank = rank; }
  );
  ...
  streamRankings.sort((a, b) => b.mean_points - a.mean_points || b.mean_marks - a.mean_marks);
  applyCompetitionRanking(
    streamRankings,
    (a, b) => Math.round(a.mean_points * 100) === Math.round(b.mean_points * 100) && Math.round(a.mean_marks * 100) === Math.round(b.mean_marks * 100),
  ...
  // 3. SUBJECT PERFORMANCE RANKING (Only for applicable subjects)
  const subjectRankings: SubjectRankingItem[] = [];
  (applicableSubjects || []).forEach((sb) => {
    const sbMarks = examMarks.filter((m) => {
      if (m.subject_id !== sb.id) return false;
    ...
    if (candidates > 0) {
      const sumMarks = sbMarks.reduce((acc, m) => {
        const evalMark = evaluateMark(m);
        return acc + evalMark.percentage!;
      }, 0);
      const meanM = Math.round((sumMarks / candidates) * 100) / 100;

      let sumPts = 0;
      sbMarks.forEach((m) => {
        const evalMark = evaluateMark(m);
        const g = getGradeForMark(evalMark.percentage!, grades);
        sumPts += g.points;
      });
  ...
  subjectRankings.sort((a, b) => b.mean_points - a.mean_points || b.mean_marks - a.mean_marks);
  applyCompetitionRanking(
    subjectRankings,
    (a, b) => Math.round(a.mean_points * 100) === Math.round(b.mean_points * 100) && Math.round(a.mean_marks * 100) === Math.round(b.mean_marks * 100),
  ...
  uniqueClassNames.forEach((cName) => {
    ...
    const classSubjects = classObj ? getAllocatedSubjectsForClass(classObj, applicableSubjects) : getApplicableSubjectsForGrade(cName, applicableSubjects);

    (classSubjects || []).forEach((sb) => {
      const sbClassMarks = examMarks.filter((m) => {
        if (m.subject_id !== sb.id || !classLearnerIds.has(m.student_id)) return false;
        const evalMark = evaluateMark(m);
        return evalMark.status === 'Normal' && evalMark.percentage !== null;
      });
      ...
      if (sbClassMarks.length > 0) {
        ...
        const meanM = Math.round((sumM / sbClassMarks.length) * 100) / 100;
  ...
  // 8. BEST SUBJECT PERFORMERS (Top 10 per subject)
  ...
  (applicableSubjects || []).forEach((sb) => {
    ...
    if (sbMarks.length > 0) {
      ...
      sbMarks.forEach((m) => {
        ...
        if (std) {
          const evalMark = evaluateMark(m);
          learnerMap.set(m.student_id, { student: std, mark: evalMark.percentage! });
        }
      });
      ...
      applyCompetitionRanking(
        topPerformers,
        (a, b) => Math.round(a.marks * 10) === Math.round(b.marks * 10),
        (item, rank) => {
          item.rank = rank;
        }
      );
  ...
  const highestClass = classRankings.length > 0 ? `${classRankings[0].class_name} (${classRankings[0].mean_marks.toFixed(1)} / ${maxObtainableMarks})` : '-';
  const highestStream = streamRankings.length > 0 ? `${streamRankings[0].full_name} (${streamRankings[0].mean_marks.toFixed(1)} / ${maxObtainableMarks})` : '-';
  const highestSubject = subjectRankings.length > 0 ? `${subjectRankings[0].subject_name} (${subjectRankings[0].mean_percentage.toFixed(1)}%)` : '-';
  const lowestSubject = subjectRankings.length > 0 ? `${subjectRankings[subjectRankings.length - 1].subject_name} (${subjectRankings[subjectRankings.length - 1].mean_percentage.toFixed(1)}%)` : '-';
