...
export const ReportsView: React.FC<ReportsViewProps> = ({
  ...
  }, [selectedExamId, selectedClassId, selectedStreamId, selectedStudentId, selectedSubjectId]);
  ...
  const isClassTeacher = currentUser?.role === 'class_teacher' || Boolean(activeTeacher?.is_class_teacher && primaryClass);
  ...
  // Auto-set and lock Class and Stream for Class Teachers on class-scoped tabs
  useEffect(() => {
    if (isClassTeacherRole && primaryClass && reportTab !== 'subject') {
  ...
  }, [isClassTeacherRole, primaryClass, reportTab, selectedClassId, selectedStreamId]);

  // Auto-select first class/stream if only 1 accessible class on subject tab
  useEffect(() => {
    if (reportTab === 'subject' && accessibleClasses.length === 1 && !selectedClassId) {
      setSelectedClassId(accessibleClasses[0].class_name);
      setSelectedStreamId(accessibleClasses[0].stream_id || accessibleClasses[0].id);
    }
  }, [reportTab, accessibleClasses, selectedClassId]);
  ...
  // Auto-select first learner when target cohort is loaded and none selected or invalid
  useEffect(() => {
    if (reportTab === 'individual' && targetStudents.length > 0) {
      if (!selectedStudentId || !targetStudents.some((s) => s.id === selectedStudentId)) {
        setSelectedStudentId(targetStudents[0].id);
      }
    }
  }, [reportTab, targetStudents, selectedStudentId]);
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2">
          {reportTab === 'batch' ? (
            ...
            <button
              ...
              disabled={isExportingProvisionalPdf || targetStudents.length === 0}
              className="bg-[#176B45] hover:bg-[#0F5132] disabled:opacity-50 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-xs transition flex items-center space-x-1.5"
            >
              ...
              <span>Export Provisional PDF</span>
            </button>
          ) : reportTab === 'individual' ? (
            <button
      ...
      <div className="print:hidden bg-white dark:bg-slate-900 rounded-xl p-3 sm:p-4 shadow-xs border border-slate-200 dark:border-slate-800 space-y-2.5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2.5 text-xs">
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (!isClassTeacherRole || reportTab === 'subject') {
                  setSelectedClassId('');
                  setSelectedStreamId('');
          ...
          {isClassTeacherRole && primaryClass && reportTab !== 'subject' ? (
            ...
            <>
              ...
              <div>
                <label className="block text-slate-700 dark:text-slate-300 font-bold mb-1">Stream</label>
                <select
                      ...
                      {sortClasses(accessibleClasses
                        ...
                        .map((c, idx) => (
                          <option key={`${c.stream_id || c.id}_${c.stream}_${idx}`} value={c.stream_id || c.id}>
                            {c.class_name} - {c.stream} ({getFilteredStudents(accessibleStudents, accessibleClasses, c.class_name, c.stream_id || c.id, selectedExam).length} learners)
                          </option>
                        ))}
                    </>
                  )}
        ...
        (() => {
          ...
          return (
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-amber-200/90 dark:border-amber-800/80 shadow-xs p-5 max-w-lg mx-auto my-6 space-y-4 text-slate-800 dark:text-slate-200">
              ...
              {(currentUser?.role === 'admin' || (currentUser?.role === 'class_teacher' && nextDestination === 'results-approval')) && (
        ...
        <>
          ...
          {!isSelectionComplete ? (
            ...
            <>
              ...
              {reportTab === 'provisional' && currentUser?.role === 'admin' && (
