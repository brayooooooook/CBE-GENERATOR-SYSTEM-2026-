        ...
        <div className="grid grid-cols-2 sm:grid-cols-6 gap-2 text-center text-xs">
          ...
          <div className="bg-white dark:bg-slate-800/80 p-2 rounded-lg border border-slate-200 dark:border-slate-700 shadow-xs dark:shadow-none">
            ...
            <div className="text-sm font-black text-amber-600 dark:text-amber-400 mt-0.5">
              {averageScore}% {!isAssessmentComplete && <span className="text-[9px] text-amber-700 dark:text-amber-300 font-medium block">(Provisional)</span>}
            </div>
          </div>
              ...
              {learnerSubjects.map((sb) => {
                ...
                return (
                  <tr key={sb.id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/60 transition ${markInfo.status === 'X' ? 'bg-rose-50/40 dark:bg-rose-950/30' : markInfo.status === 'Y' ? 'bg-amber-50/40 dark:bg-amber-950/30' : ''}`}>
                        ...
                        <span
                          className={`px-1.5 py-0.5 rounded text-[10px] font-bold inline-block border ${
                            levelStr === 'EE'
                              ...
                              : levelStr === 'X'
                              ? 'bg-rose-100/80 dark:bg-rose-950/50 text-rose-800 dark:text-rose-300 border-rose-300 dark:border-rose-800 font-bold'
                              : 'bg-amber-100/80 dark:bg-amber-950/50 text-amber-800 dark:text-amber-300 border-amber-300 dark:border-amber-800 font-bold'
                          }`}
                        >
                          {levelStr}
                        </span>
                    ...
                    <td className="p-2 text-center border-r border-slate-300 dark:border-slate-700 font-extrabold text-slate-700 dark:text-slate-300 text-[11px]">
                      {gradeCodeStr}
                    </td>
