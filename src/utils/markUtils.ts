...
/**
 * Consistently rounds a percentage value to max 1 decimal place, returning a number.
 * E.g., 56.00000000000001 -> 56, 56.333333 -> 56.3
 */
export function roundPercentage(val: number | string | null | undefined): number | null {
  if (val === null || val === undefined || val === '') return null;
  const num = typeof val === 'number' ? val : parseFloat(String(val));
  if (isNaN(num)) return null;
  return Number((Math.round((num + Number.EPSILON) * 10) / 10).toFixed(1));
}
